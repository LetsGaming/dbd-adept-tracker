<template>
  <div class="max-w-[1200px] mx-auto px-4 pb-24 pt-4">
    ...
    <!-- Table -->
    <div
      class="rounded-xl border border-[var(--color-border-subtle)] overflow-x-auto"
    >
      <table class="w-full border-collapse text-sm">
        <thead>
          <tr>
            <!-- Portrait column — not sortable -->
        ...
        <tbody>
          ...
          >
            ...
            <td
              class="px-3 py-2.5 text-center text-xs text-[var(--color-text-muted)]"
            >
            ...
            <td class="px-3 py-2.5 text-center">
              <span
                ...
                style="
                  background: var(--color-accent, #22d3ee) 18;
                  border-color: var(--color-accent, #22d3ee) 40;
...
export default defineComponent({
  ...
  data() {
    return {
      columns: [
        { key: "name" as StatsSortCol, label: "Name" },
        { key: "side" as StatsSortCol, label: "Seite" },
        { key: "tries" as StatsSortCol, label: "Tries" },
        { key: "doneAt" as StatsSortCol, label: "Datum" },
        { key: "status" as StatsSortCol, label: "Status" },
      ],
    };
  },
  computed: {
    ...
    summaryCards() {
      ...
      const done = this.allRows.filter((r) => r.done).length;
      const inProgress = this.allRows.filter(
        (r) => r.tries > 0 && !r.done,
      ).length;
      const totalTries = this.allRows.reduce((s, r) => s + r.tries, 0);
      return [
